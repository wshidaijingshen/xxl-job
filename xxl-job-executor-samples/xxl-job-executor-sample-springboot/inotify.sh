...
monitor_file4=/opt/config/application.yml
monitor_file5=/opt/config/application-prod.yml

log_file=/opt/logs/inotify.log

echo $(date +"%Y-%m-%d %H:%M:%S") 等待中 $monitor_file1,$monitor_file2,$monitor_file3,$monitor_file4,$monitor_file5 >> $log_file
sleep 15
echo $(date +"%Y-%m-%d %H:%M:%S") 监听中 $monitor_file1,$monitor_file2,$monitor_file3,$monitor_file4,$monitor_file5 >> $log_file

inotifywait  -o $log_file  -e attrib,create \
  --timefmt '%Y-%m-%d %H:%M:%S' --format '%T %w%f %e' \
  $monitor_file1 $monitor_file2 $monitor_file3 $monitor_file4 $monitor_file5
